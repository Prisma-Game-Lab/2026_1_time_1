...
public class NezhaAIHard : MonoBehaviour
{
    [Header("References")]
    ...
    [Header("Chute apos cada ataque")]
    [Tooltip("Se marcado, dispara o Chute depois de todo ataque escolhido pela IA.")]
    [SerializeField] private bool chuteAposCadaAtaque = true;
    [Tooltip("Se marcado, NAO dispara o chute extra quando o proprio ataque escolhido ja foi o Chute (evita chute -> chute).")]
    [SerializeField] private bool pularChuteSeAtaqueJaFoiChute = true;
    ...
    // Indice do ataque Chute no switch (case 1).
    private const int CHUTE = 1;
    ...
    private void Start()
    {
        ...
        // Resolve as referencias por tipo se algum slot ficou vazio no Inspector.
        // Isso evita "ataque errado" quando a referencia do Chute nao foi arrastada.
        ResolverReferencias();
    ...
    // Puxa cada modulo do proprio boss (mesmo GameObject) quando o slot estiver vazio.
    // Como cada tipo e distinto (NezhaChute, NezhaFirewall, ...), nao tem como pegar o ataque errado.
    private void ResolverReferencias()
    {
        if (movement == null) movement = GetComponent<NezhaMovement>();
        if (fireballAttack == null) fireballAttack = GetComponent<NezhaFireball>();
        if (chuteAttack == null) chuteAttack = GetComponent<NezhaChute>();
        if (slamAttack == null) slamAttack = GetComponent<NezhaSlam>();
        if (firewallAttack == null) firewallAttack = GetComponent<NezhaFirewall>();
        if (animator == null) animator = GetComponent<Animator>();

        if (chuteAttack == null)
            Debug.LogError("[NezhaAIHard] NezhaChute nao encontrado no boss. O chute apos cada ataque nao vai funcionar. " +
                           "Arraste o componente NezhaChute no slot 'Chute Attack' ou confira se ele esta no mesmo GameObject.", this);
    }
    ...
    private IEnumerator MainLoop()
    {
        while (true)
        {
            ...
            // Ataque escolhido pela IA (logica de escolha inalterada).
            yield return ExecutarAtaque(attack);

            // Chute forcado depois de CADA ataque -> chama o NezhaChute DIRETO (sem indice, sem confusao).
            if (chuteAposCadaAtaque && chuteAttack != null &&
                !(pularChuteSeAtaqueJaFoiChute && attack == CHUTE))
            {
                // Pequena pausa neutra pra reposicionar/encarar o player antes do chute.
                yield return IdlePhase();
                chuteAttack.Iniciar();
    ...
    // Executa um ataque pelo indice e espera ele terminar.
    // Nao mexe em lastAttack nem nos pesos: e so o disparo/espera.
    private IEnumerator ExecutarAtaque(int attack)
    {
        switch (attack)
        {
            case 0:
                if (fireballAttack == null) yield break;
                fireballAttack.Iniciar();
                yield return new WaitUntil(() => !fireballAttack.IsAttacking);
                break;

            case 1:
                if (chuteAttack == null) yield break;
                chuteAttack.Iniciar();
                yield return new WaitUntil(() => !chuteAttack.IsAttacking);
                break;

            case 2:
                if (slamAttack == null) yield break;
                slamAttack.Iniciar();
                yield return new WaitUntil(() => !slamAttack.IsAttacking);
                break;

            case 3:
                if (firewallAttack == null) yield break;
    ...
    private int PickAttack()
    {
        ...
        // Injeta aleatoriedade controlada para nao ficar exploravel.
        float avg = 0f;
